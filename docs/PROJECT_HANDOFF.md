...
> 本文档是「地图」，那 5 份是「地形」。
>
> **快照时间**：2026-05-29 · **基线分支**：`master` · **最新基线 commit**：
> `8b38a13 introduce typed ApiError and form-level field error binding`
> （Phase 1–4 沙箱改动作为 baseline dirty files 存在工作区，尚未提交，详见 §6.2）
...
- **现在所在阶段**：Phase 0「稳固基础」最后一公里。沙箱已从骨架打通到真实 go-judge
  HTTP 调用（Phase 1–4 完成），就差**多语言运行时镜像 + 端到端联调**（Phase 5）。
- **最紧迫的一件事**：交付包含 Python / C++ / Java 三套 runtime 的可复现 sandbox
  生产镜像，并跑通"学生提交 → 评测 → 详情展示"全链路。完成这个，Phase 0 才算收尾，
  Phase 1（用户组）才能开。
- **不能动的红线**（违反任意一条都会埋雷）：
...
| 项 | 状态 | 落地缺口 |
|---|---|---|
| 真实隔离沙箱（Phase 5） | 🟡 4/5 phase 完成 | 多语言 runtime image + E2E 联调 |
...
> 当前 `master` HEAD 是 `8b38a13`。Phase 1–4 沙箱体系的**全部代码改动**作为
> baseline dirty files 存在工作区，**尚未提交**。每个 phase 的 inbox/outbox 在
> `docs/codex-exchange/`，可逐 phase 复盘。

接手第一件事：用 `git status` 看清楚工作区有什么、用 `git diff` 浏览改动面，
**决定是否一次性提交 Phase 1–4 成果**（推荐：按 phase 拆 4 个 commit，保留可追溯性）。
...
- [ ] 重构 `deploy/sandbox/Dockerfile`，固化包含 `/usr/bin/python3` / `/usr/bin/g++` /
      `/usr/bin/javac` / `/usr/bin/java` 的可复现镜像（指定 apt 镜像源、apt 包版本固定、
      最终 image 体积可控）
- [ ] 启 Phase 5 任务：由 Codex 直接设计并实施 multi-language runtime image
- [ ] 端到端联调清单（手动）：
  - Python `print('hello')` → ACCEPTED + maxTime / maxMemory > 0
  - Python `while True: pass` → TIME_LIMIT_EXCEEDED
  - Python `import sys; sys.exit(7)` → RUNTIME_ERROR + exitStatus=7
  - C++ 编译错误 → COMPILE_ERROR + stderrExcerpt 含报错
  - Java `Main.java` 单类 → ACCEPTED
  - 学生端详情弹窗：runTime / exitStatus / stderr 非空时正确渲染
- [ ] 把 Phase 1–4 工作区改动拆成 4 个 commit 入仓
- [ ] Flyway V7 在干净 DB 上跑通；在带 V1–V6 历史的 DB 上跑通
...
| 风险 | 严重度 | 缓解 |
|---|---|---|
| Phase 5 sandbox 多语言镜像未交付 | 🔴 P0 | 阻塞 Phase 0 收尾，直接影响"能否上线" |
| 后端单测为空 | 🔴 P0 | 任何重构都没有保护网；先补 4 个关键 service |
| Phase 1–4 改动未提交 | 🟠 P1 | 工作区脏，丢机即丢沙箱链路成果 |
...
### Day 4 · 起 Phase 5
- [ ] 由 Codex 直接启动 Phase 5 multi-language runtime image 任务
- [ ] 调研 `deploy/sandbox/Dockerfile` 当前内容，列 Phase 5 需要补的 apt 包清单
- [ ] 设计：apt 镜像源（中国镜像或 host 提供）/ 包版本固定策略 / 最终 image 大小目标
- [ ] 起 Phase 5（按 §8.2 phase 纪律）

### Day 5 · 跑 E2E 联调 6 条用例
按 §7.1 Week 1–2 的清单，逐条手动跑通 6 条 sandbox 用例。
...
### Day 7 · 复盘 + 更新文档
- [ ] 更新 `HANDOVER.md §2` 把 Phase 5 移到已完成（如果跑通）
- [ ] 更新 `ROADMAP.md` Phase 0 状态
- [ ] 在本文档 §6.3 把"Phase 1–4 工作区未提交"标记移除
